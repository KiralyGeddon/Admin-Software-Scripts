...
check_and_install_docker() {
    ...
    then
        ...
        if [ $? -eq 0 ]; then
            ...
            bash /home/sam/script/TSSR/logiciels/scripts/install_docker.sh
            if [ $? -eq 0 ]; then
...
sous_menu_docker() {
    ...
    case $CHOIX in
        1) logiciel="portainer" && installer "Portainer" && bash /home/sam/script/TSSR/logiciels/scripts/install_$logiciel.sh && installer1 $logiciel ;;
        2) logiciel="glpi" && installer "GLPI (Gestion de parc)" && bash /home/sam/script/TSSR/logiciels/scripts/install_$logiciel.sh && installer1 $logiciel ;;
        3) logiciel="zabbix" && installer "Zabbix (Supervision)" && bash /home/sam/script/TSSR/logiciels/scripts/install_$logiciel.sh && installer1 $logiciel ;;
        4) logiciel="nagios" && installer "Nagios (Supervision)" && bash /home/sam/script/TSSR/logiciels/scripts/install_$logiciel.sh && installer1 $logiciel ;;
        5) logiciel="xivo" && installer "XiVO (VOIP)" && bash /home/sam/script/TSSR/logiciels/scripts/install_$logiciel.sh && installer1 $logiciel ;;
        6) return ;;
...
main(){
    ...
    chmod +x /home/sam/script/TSSR/logiciels/scripts/*.sh 2>/dev/null || true
    menu_principal
}
