...
# =============================================================================
# Fonction: calculate_netmask_from_cidr
# Calcule le masque de sous-réseau à partir d'une adresse IP en notation CIDR (ex: 192.168.1.10/24).
# Nécessite le paquet 'ipcalc'.
# Retourne le masque de sous-réseau ou une chaîne vide en cas d'erreur.
# =============================================================================
calculate_netmask_from_cidr() {
    local ip_cidr="$1"
    local netmask=""

    # Assurez-vous que ipcalc est installé (bien que install_package_if_not_exists le fasse en amont, c'est une sécurité)
    if ! command -v ipcalc &> /dev/null; then
        print_error "Le paquet 'ipcalc' est nécessaire pour calculer le masque de sous-réseau mais n'est pas installé."
        return 1
    fi

    # Utilise ipcalc pour extraire le masque de sous-réseau
    # La sortie de 'ipcalc --nobreak -n' pour '192.168.1.1/24' est '255.255.255.0'
    netmask=$(ipcalc --nobreak -n "$ip_cidr" 2>/dev/null)

    if [[ -z "$netmask" ]]; then
        print_error "Impossible de calculer le masque de sous-réseau pour '$ip_cidr'. Vérifiez le format de l'IP/CIDR."
        return 1
    fi
    echo "$netmask"
    return 0
