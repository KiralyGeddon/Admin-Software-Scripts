...
display_header() {
    ...
    local header_lines=(
        ...
        "            Babak - Raulyna - Ounayss - Samuel.V - Yusuf"
...
maj(){
    ...
    # Exécute la mise à jour en arrière-plan (apt update et apt upgrade).
    if [[ "$(whoami)" == "root" ]]; then
    ...
    # Affiche une barre de progression pendant que la mise à jour est en cours.
    progress_bar "Mise à jour du système en cours..." 10 # Durée de 10 secondes pour la barre.
    
    # Attend que le processus apt se termine.
    wait $apt_pid
    ...
    if [ $apt_status -eq 0 ]; then
        ...
        print_error "Échec de la mise à jour du système."
